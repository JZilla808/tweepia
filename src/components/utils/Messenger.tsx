...
import { TweepiaFileEvent, TweepiaInputEvent } from "../../types/Form";
import { Tweep } from "../../types/Tweep";
import Button from "./Button";

type Props = {
  onTweepAdded: (t: Tweep | undefined) => void;
  showAvatar?: boolean;
  replyTo?: string;
};
...
const Messenger: Component<Props> = (initialprops) => {
  ...
  const { handleInput, handleSubmit, form, loading, image, setImage } =
    useMessenger(props.replyTo);

  const sendDisabled = () => loading() || form.content === "";

  const autosize = (e: TweepiaInputEvent) => {
  ...
  const handleImageSelection = (e: TweepiaFileEvent) => {
    if (e.target.files) {
        ...
        <div class="flex-it mb-1 flex-row xs:justify-between items-center">
          <div class="flex-it mt-3 mr-3 cursor-pointer text-white hover:text-blue-400 transition">
            <div class="upload-btn-wrapper">
              <FaRegularImage class="cursor-pointer" fill="white" size={18} />
              <input
          ...
          <div class="flex-it w-32 mt-3 cursor-pointer">
            <Button
              ...
              onClick={async () => {
                const tweep = await handleSubmit();
                props.onTweepAdded(tweep);
              }}
            >
              <span>Tweep It</span>
            </Button>
